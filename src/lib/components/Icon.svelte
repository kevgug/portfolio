<script lang="ts" context="module">
  const icons = {
    "arrow-down": {
      box: 24,
      svg: `<defs></defs><line x1="12" y1="0.75" x2="12" y2="23.25" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5px"></line><polyline points="1.5 12.75 12 23.25 22.5 12.75" fill-rule="evenodd" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5px"></polyline>`,
    },
    "chevron-down": {
      box: 24,
      svg: `<polyline points="6 9 12 15 18 9" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></polyline>`,
    },
    "chevron-up": {
      box: 24,
      svg: `<polyline points="6 15 12 9 18 15" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></polyline>`,
    },
    "network-pin": {
      box: 24,
      svg: `<path stroke="currentColor" stroke-width="1.5" d="M18.75 16.1988C18.5429 16.1988 18.375 16.0309 18.375 15.8238C18.375 15.6166 18.5429 15.4488 18.75 15.4488"></path><path stroke="currentColor" stroke-width="1.5" d="M18.75 16.1988C18.9571 16.1988 19.125 16.0309 19.125 15.8238C19.125 15.6166 18.9571 15.4488 18.75 15.4488"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M18.75 11.3238C19.9435 11.3238 21.0881 11.7979 21.932 12.6418C22.7759 13.4857 23.25 14.6303 23.25 15.8238C23.25 17.7448 20.562 21.4008 19.341 22.9618C19.2709 23.0515 19.1812 23.1242 19.0788 23.1741C18.9764 23.224 18.8639 23.25 18.75 23.25C18.6361 23.25 18.5236 23.224 18.4212 23.1741C18.3188 23.1242 18.2291 23.0515 18.159 22.9618C16.938 21.3998 14.25 17.7448 14.25 15.8238C14.25 14.6303 14.7241 13.4857 15.568 12.6418C16.4119 11.7979 17.5565 11.3238 18.75 11.3238Z"></path><g><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M14.2491 23.0229C12.0708 23.4673 9.8096 23.2564 7.75105 22.4168C5.6925 21.5771 3.9289 20.1463 2.68289 18.3051C1.43689 16.4638 0.764367 14.2947 0.750228 12.0716C0.736088 9.84839 1.38097 7.67087 2.60345 5.81394C3.82593 3.95701 5.5712 2.50392 7.6189 1.63813C9.66659 0.772343 11.9249 0.532681 14.1087 0.949404C16.2925 1.36613 18.3039 2.42055 19.8889 3.97954C21.4739 5.53853 22.5614 7.53219 23.0142 9.7088"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.29231 22.9119C7.77099 20.6809 6.75444 16.6267 6.75444 11.9958C6.75444 7.36485 7.77099 3.31165 9.29231 1.07964"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M0.782089 11.2461H12.7518"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.0051 5.24879H20.9981"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.05453 17.2435H11.2524"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M14.7119 1.07964C16.007 3.26798 16.7956 5.71853 17.0199 8.25145"></path></g>`,
    },
    "arrow-corner-right": {
      box: 24,
      svg: `<defs></defs><line x1="4" y1="3.75" x2="19.91" y2="19.66" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5px"></line><polyline points="4.91 19.66 19.91 19.66 19.91 4.66" fill-rule="evenodd" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5px"></polyline>`,
    },
    "house-entrance": {
      box: 24,
      svg: `<defs></defs><path d="M22.868,8.947,12,.747,1.122,8.947a1.177,1.177,0,0,0-.377.8V22.269a.981.981,0,0,0,.978.978H8.245V18a3.75,3.75,0,0,1,7.5,0v5.25h6.521a.982.982,0,0,0,.979-.978V9.747A1.181,1.181,0,0,0,22.868,8.947Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5px"></path>`,
    },
    "graduate-cap": {
      box: 24,
      svg: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="-0.75 -0.75 24 24" height="24" width="24" id="Graduation-Cap--Streamline-Plump"><desc>Graduation Cap Streamline Icon: https://streamlinehq.com</desc><g id="Graduation-Cap--Streamline-Plump"><path id="Vector 2536" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m2.34375 8.4375 0 8.90625" stroke-width="1.5"></path><path id="Rectangle 1097" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M1.430625 7.76015625c-0.6478124999999999 -0.3103125 -0.6478124999999999 -1.1475 0 -1.4578125 0.849375 -0.406875 2.11828125 -0.984375 3.9131250000000004 -1.73203125 2.4937500000000004 -1.03875 4.1971875 -1.6546874999999999 5.11171875 -1.966875a2.4515625 2.4515625 0 0 1 1.5890625 0c0.9140625 0.3121875 2.6184375 0.928125 5.11171875 1.966875 1.7953125 0.74765625 3.0637499999999998 1.32515625 3.91359375 1.73203125 0.646875 0.3103125 0.646875 1.1475 0 1.4578125 -0.84984375 0.406875 -2.11875 0.984375 -3.91359375 1.73203125 -2.4937500000000004 1.03875 -4.1971875 1.6546874999999999 -5.11171875 1.966875a2.4515625 2.4515625 0 0 1 -1.5890625 0c-0.9140625 -0.3121875 -2.6184375 -0.928125 -5.11171875 -1.966875 -1.7953125 -0.748125 -3.0637499999999998 -1.32515625 -3.9131250000000004 -1.73203125Z" stroke-width="1.5"></path><path id="Subtract" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M5.2040625 9.43359375A35.0840625 35.0840625 0 0 0 5.15625 11.3278125c0 1.4503125 0.07453125000000001 2.3610937499999998 0.14765625 2.90625 0.06421875 0.47625 0.33703125 0.88125 0.76265625 1.10390625C6.8896875 15.769218749999999 8.56171875 16.40625 11.25 16.40625c2.68828125 0 4.3603125 -0.6365625 5.1834375 -1.0678125 0.42562500000000003 -0.22265625 0.6984375 -0.62765625 0.76265625 -1.1034375 0.073125 -0.5456249999999999 0.14765625 -1.4564062500000001 0.14765625 -2.90625a35.0765625 35.0765625 0 0 0 -0.047812499999999994 -1.8946874999999999" stroke-width="1.5"></path><path id="Ellipse 20" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M3.75 18.75a1.40625 1.40625 0 1 1 -2.8125 0 1.40625 1.40625 0 1 1 2.8125 0" stroke-width="1.5"></path></g></svg>`,
    },
    phone: {
      box: 24,
      svg: `<path d="m15.04 22.39 0.012 0.007a5.533 5.533 0 0 0 6.884 -0.755l0.774 -0.774a1.846 1.846 0 0 0 0 -2.609L19.449 15a1.846 1.846 0 0 0 -2.609 0h0a1.843 1.843 0 0 1 -2.608 0L9.014 9.781a1.846 1.846 0 0 1 0 -2.609h0a1.843 1.843 0 0 0 0 -2.608L5.754 1.3a1.846 1.846 0 0 0 -2.609 0l-0.774 0.774a5.535 5.535 0 0 0 -0.756 6.884l0.008 0.012A49.935 49.935 0 0 0 15.04 22.39Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    email: {
      box: 24,
      svg: `<path d="m1.5 4.75 21 0 0 15 -21 0Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="m22.161 5.3 -8.144 6.264a3.308 3.308 0 0 1 -4.034 0L1.839 5.3" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    person: {
      box: 24,
      svg: `<path d="M0.5 23a10.135 10.135 0 0 1 1 -4.057c0.705 -1.41 3.65 -2.387 7.347 -3.756 1 -0.371 0.836 -2.981 0.393 -3.468C8.2 10.576 7.32 9.235 7.32 6c0 -3.265 2.13 -5 4.68 -5s4.68 1.735 4.68 5c0 3.235 -0.881 4.576 -1.92 5.719 -0.443 0.487 -0.607 3.1 0.393 3.468 3.7 1.369 6.642 2.346 7.347 3.756a10.135 10.135 0 0 1 1 4.057Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1"></path>`,
    },
    wave: {
      box: 24,
      svg: `<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="M4.69446 10.7499c-0.6 -0.6 -1.6 -0.6 -2.3 0 -0.6 0.6 -0.6 1.6 0 2.3l8.60004 8.6c1 1 2.4 1.6 3.9 1.6s2.9 -0.6 3.9 -1.6l2.2 -2.2c1.5 -1.5 2.2 -3.7 1.8 -5.8l-1.4 -6.89997c-0.1 -0.7 -0.6 -1.2 -1.3 -1.3 -0.9 -0.2 -1.9 0.5 -2 1.4l-0.4 3.29997 -7.4 -7.39997c-0.60004 -0.6 -1.60004 -0.6 -2.30004 0 -0.6 0.6 -0.6 1.6 0 2.3l-1 -1.1c-0.6 -0.6 -1.6 -0.6 -2.3 0 -0.6 0.6 -0.6 1.6 0 2.3l1.1 1.1c-0.6 -0.6 -1.6 -0.6 -2.3 0 -0.6 0.6 -0.6 1.6 0 2.3l1.2 1.09997Z" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="m7.99292 5.04602 4.49998 4.5" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="m5.79449 7.34991 4.60001 4.49999" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="m8.09446 14.1499 -3.4 -3.4" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="M16.3945 3.25c0.4 0 0.7 0.1 1 0.3" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="M16.8945 0.75c1.1 0 2.1 0.4 2.8 1.2" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="M3.59448 18.75c0 1.1 0.9 2 2 2" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="M1.09448 19.25c0 2.2 1.8 4 4 4" stroke-width="1.5"></path>`,
    },
    "multiple-neutral-2": {
      box: 24,
      svg: `<path d="M3.375 7.875a4.125 4.125 0 1 0 8.25 0 4.125 4.125 0 1 0 -8.25 0Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="M0.75 20.25a6.75 6.75 0 0 1 13.5 0" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="M14.352 10.125a3.375 3.375 0 1 0 6.75 0 3.375 3.375 0 1 0 -6.75 0Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="M15.813 15.068a5.526 5.526 0 0 1 7.437 5.182" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    "book-open": {
      box: 24,
      svg: `<path d="M11.458 21.269a1.6 1.6 0 0 0 1.085 0 13.66 13.66 0 0 1 8.7 -0.448 1.607 1.607 0 0 0 2.006 -1.559V4.928a1.607 1.607 0 0 0 -1.12 -1.528 13.878 13.878 0 0 0 -9.586 0.2 1.61 1.61 0 0 1 -1.085 0 13.881 13.881 0 0 0 -9.587 -0.2A1.607 1.607 0 0 0 0.75 4.928v14.334a1.607 1.607 0 0 0 2.006 1.559 13.663 13.663 0 0 1 8.702 0.448Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="m12 3.682 0 17.682" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    house: {
      box: 24,
      svg: `<path d="M22.868 8.947 12 0.747l-10.878 8.2a1.177 1.177 0 0 0 -0.377 0.8v12.522a0.981 0.981 0 0 0 0.978 0.978h6.522V18a3.75 3.75 0 0 1 7.5 0v5.25h6.521a0.982 0.982 0 0 0 0.979 -0.978V9.747a1.181 1.181 0 0 0 -0.377 -0.8Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    bell: {
      box: 24,
      svg: `<path d="M15.875 5.827a3 3 0 1 0 6 0 3 3 0 1 0 -6 0" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="M9.125 21.75a2.087 2.087 0 0 0 4.005 0" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="m11.125 3 0 -2.25" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="M18.648 11.961c0.189 5.756 1.477 6.789 1.477 6.789h-18s1.5 -1.916 1.5 -8.25a7.507 7.507 0 0 1 9.724 -7.165" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    "arrow-left": {
      box: 24,
      svg: `<path d="M23.25 12 0.75 12" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path><path d="M11.25 1.5 0.75 12l10.5 10.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"></path>`,
    },
    "arrow-up": {
      box: 24,
      svg: `<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M1.65448 23.0773c4.14279 -0.2844 8.02223 -2.1342 10.85112 -5.1741 2.8289 -3.0399 4.3953 -7.0422 4.3814 -11.19467 0 -0.27665 0.014 -0.93855 0 -1.21441l0 -4.525371" stroke-width="1.5"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="M11.4409 6.375 16.8932 0.922705 22.3455 6.375" stroke-width="1.5"></path>`,
    },
  } as const;
  export type IconName = keyof typeof icons;
</script>

<script lang="ts">
  export let name: IconName;
  export let size = "16px";
  export let flipY = false;

  interface Icon {
    box: number;
    svg: string;
  }

  const defaultIcon: Icon = {
    box: 12,
    svg: ``,
  };

  let displayIcon = icons[name] ?? defaultIcon;
</script>

<span style={flipY ? "transform: scaleY(-1);" : ""}>
  <svg
    class={$$props.class ?? ""}
    style="overflow: visible;"
    width={size}
    height={size}
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 {displayIcon.box} {displayIcon.box}"
  >
    {@html displayIcon.svg}
  </svg>
</span>
